...
% Next steps involve:
%
% * Potentially using interpolation to allow smoothing of outer mesh to inner
...
%specified thickness
%==2: Adds a thin shell element at specified height
...
Chamber_wt=1; % wall thickness expanding (mm)
Gap_length=1.8; %(mm)
...
Gap_length_elements=2; % no of mesh elements on this length

% y direction 

Side_wall_thickness_elements=1; % no of mesh elements on this length
Chamber_width_elements=8; % no of mesh elements on this length (Inclusive of channel width)
...
SLL_thickness_elements=2; %Elements used in SLL layer if method 1 applied

Mat1_base_thickness_elements=2; %mat 1 basse layer thickness (mm)
Channel_height_elements=3; %height of internal channel between chambers (mm)
...
logicContactPrimary=Normals(:,1)==1;%where norm v is pos x 
logicContactSecondary=Normals(:,1)==-1;%where norm v is neg x 

logicContactPrimary=logical(logicContactSurf.*logicContactPrimary);
logicContactSecondary=logical(logicContactSurf.*logicContactSecondary);

F_contactPrimary=Fb(logicContactPrimary,:);
F_contactSecondary=Fb(logicContactSecondary,:);

cFigure;
gpatch(F,V,'bw','k',0.05);
hold on;
gpatch(F_contactPrimary,V,'rw','k');
gpatch(F_contactSecondary,V,'y','k');
axisGeom;
...
gpatch(Fb,V,Cb,'none',0.05);%normally set transp to 0.5, 0.05 shows pressure face better
...
gpatch(F,V,'bw','b',0.05,1);
gpatch(F_pressure,V,'rw','k',1);
scatterV(V(indInterp,:),100,Umag_indInterp,'filled')

axisGeom(gca,fontSize); 
camlight headlight; colormap spectral; hcb=colorbar;
hcb.Title.String = "Displacement (mm)";
drawnow; 
...
%Get spine faces
FSLL=Fb(Cb==5,:);%gets spine face

% dirSet=1; % direction to extrude some elements
% [E2,V2]=patchThick(FSLL,V,dirSet,10,2);
% F2=element2patch(E2,[],'hex8');
% 
% % Join node sets
...
layerThickness=0.5; 
numSteps=3; 
[E2,V2,Fq1,Fq2]=quadThick(FSLL,V,1,SLL_thickness,SLL_thickness_elements+1);

%Use element2patch to get patch data 
[F2,~,faceSideType]=element2patch(E2);
E2=E2+size(V,1); %Offset indices for hexa 
V=[V;V2];
Fb=[Fb;F2];
E1=E;
E=[E1;E2];
CSLL=ones(1,size(F2,1))*(1+(max(Cb)));
Cb=[Cb;CSLL'];

elseif SLL_control == 2 % Shell SLL material

% logicSLL=V(:,3)== min(V(:,3));
% VSLL=V(logicSLL,:);
% VSLL(:,3)= min(V(:,3))+SLL_thickness;
logic_FSLL=Cb==5;
FSLL=Fb(logic_FSLL,:);
% F2=FSLL+max(Fb(:));
% Fb=[Fb; F2];%Combines the SLL faces into Fb
% CSLL=ones(1,size(F2,1))*(1+(max(Cb)));
% Cb=[Cb;CSLL'];
...
febio_spec.Mesh.Elements{2}.elem.ATTR.id=size(E,1)+(1:1:size(FSLL,1))'; %Element id's
febio_spec.Mesh.Elements{2}.elem.VAL=FSLL; %The element matrix
end
...
febio_spec.Mesh.Surface{2}.quad4.ATTR.id=(1:1:size(F_contactPrimary,1))';%size(F_pressure,1)+
febio_spec.Mesh.Surface{2}.quad4.VAL=F_contactPrimary;

surfaceName3='contactSurface2';
febio_spec.Mesh.Surface{3}.ATTR.name=surfaceName3;
febio_spec.Mesh.Surface{3}.quad4.ATTR.id=(1:1:size(F_contactSecondary,1))';%size(F_pressure,1)+size(F_contactPrimary,1)+
febio_spec.Mesh.Surface{3}.quad4.VAL=F_contactSecondary;
...
febio_spec.MeshData.ElementData{1}.elem.ATTR.lid=(1:1:size(FSLL,1))';
febio_spec.MeshData.ElementData{1}.elem.VAL=SLLThicknessShell*ones(size(FSLL,1),size(FSLL,2));
end
