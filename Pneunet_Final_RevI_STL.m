...
Chamber_height=20; %internal height of the chamber (mm)

%% Object controls

% Soft object controls
ObjectCentreDist=[0 0 -20]; % distance from Pneuent tip to object centre (mm)
...
SoftObject_PointSpacing=3; %mesh refinement parameter
SoftObjectLength=15;
SoftObjectWidth=30;
...
q=q+1;
 hll1=gpatch(ContactPair.Primary{q},V,'rw','k');
 hll2=gpatch(ContactPair.Secondary{q},V,'y','k');
legend([hll1,hll2],'Primary surfaces','Secondary surfaces')
axisGeom;
...
E=[E1;E2];
FE2=element2patch(E2,[],'hex8');
cFigure;
gpatch(Fb,V,Cb,'k',0.5);%0.5 transperancy normally hold on
hold on;  hll=gpatch(FE2,V,'g'); % SLL elements
legend(hll,'SLL elements')
axisGeom;
...
cFigure;
gpatch(Fb,V,Cb,'k',0.5);%0.5 transperancy normally hold on
hold on; hll=gpatch(E{2},V,'g'); % SLL shell elements
...
if object_control==2
    ...
    pointSpacingRigid=3; %Desired point spacing
    ...
    hll1=gpatch(Fb,V,'bw');
    hold on;
    hll2=gpatch(F_rigid, V, 'r','k')
    hll3=plotV(V(bcSupportList,:),'k.','MarkerSize',15);
    colormap(turbo(250)); %icolorbar; 
    %camlight headlight; 
    legend([hll1,hll2,hll3],'Pneu-net','Rigid surface','BC full support');
    axisGeom; 
...
%     icolorbar;
    ...
    hll1=gpatch(Fb,V,'bw');
    hold on;
    hll2=gpatch(Fbso,Vso,'y','k')
    hll3=plotV(V(bcSupportList,:),'k.','MarkerSize',15);
    colormap(turbo(250)); %icolorbar; 
    %camlight headlight; 
    legend([hll1,hll2,hll3],'Pneu-net','Object','BC full support');
    axisGeom; 
...
%     axisGeom;
cFigure;
    hll1=gpatch(Fb,V,'bw','k',1);
    hold on;
    hll2=gpatch(Fbso,Vso,'rw','k')
    hll3=plotV(V(bcSupportList,:),'k.','MarkerSize',15);
    hll4=plotV(V(bcMidPlane,:),'b.','MarkerSize',15);
    colormap(turbo(250)); %icolorbar; 
    %camlight headlight; 
    legend([hll1,hll2,hll3,hll4],'Pneu-net','Object','BC full support','Object BC full support');
    axisGeom; 
   
    
    cFigure;
    hll1=gpatch(Fb,V,'bw','k',0.1);
    hold on;
    %hll2=gpatch(Fbso(Cbso~=6 | Cbso~=1 | Cbso~=2,:),Vso,'g','k',0.1)
    hll3=gpatch(contactObjectPrimary,V,'rw','k',1);
    hll4=gpatch(contactObjectSecondary,V,'y','k',1);
    colormap(turbo(250)); %icolorbar; 
    %camlight headlight; 
    legend([hll3,hll4],'Primary surface','Secondary surface');
    axisGeom; 
...
subplot(1,3,2);
gpatch(Fb_bottomMould,V_STLBase,'y');
axisGeom;
