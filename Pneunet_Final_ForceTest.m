...
STL_TopMould=fullfile(savePath,[febioFebFileNamePart,'_TopMould.stl']); %Log file name for exporting .stl
...
max_refs=opt_iter*5; %Max reforms
max_ups=0; %Set to zero to use full-Newton iterations
max_retries=15;%10 %Maximum number of retires
...
%% Load Inputs
%Load
designPressureAngleV=[0.01 0.02 0.04 0.06 0.08];

designPressureForce=0.1;

forceData=cell(size(designPressureAngleV,2),4,6);
...
for geomCounter=1:1:6
    if geomCounter==2
        SLL_thickness=2
    elseif geomCounter==3
        Side_wall_thickness=4;
    elseif geomCounter==4
        Chamber_length=15
    elseif geomCounter==5
        n=6
    elseif geomCounter==6
        Channel_roof_thickness=1.5
    end
    
for force_counter=1:1:size(designPressureAngleV,2)
designPressureAngle=designPressureAngleV(force_counter); %(MPa)-target pressure for required bend
%% Object controls
...
forceData{force_counter,1,geomCounter}=Force_total;
forceData{force_counter,2,geomCounter}=timeVec;

indT1=find(timeVec==1); %index at which step 2 starts
nodeEnd=mean(V_DEF(indForceNodes,:,indT1),1);%averaged end Node tracking over time
forceData{force_counter,3,geomCounter}=nodeEnd;
forceData{force_counter,4,geomCounter}=BendingAngle(indT1,2);
end

cFigure;
plot(forceData{1,2,geomCounter},forceData{1,1,geomCounter},'k')
hold on;
plot(forceData{2,2,geomCounter},forceData{2,1,geomCounter},'b')
plot(forceData{3,2,geomCounter},forceData{3,1,geomCounter},'r')
plot(forceData{4,2,geomCounter},forceData{4,1,geomCounter},'g')
plot(forceData{5,2,geomCounter},forceData{5,1,geomCounter},'y')
title('Force versus stopped distance');
xlabel('Distance (mm)'); ylabel('Total force (N)');


end
%%

forceDataNew=forceData;

NewFData=zeros(2,1,6);

for i=1:1:size(forceData,3)
    for j=1:1:size(forceData,1)
forceDataNew{j,3,i}=abs(min(forceDataNew{j,3,i}(:,3)))

forceDataNew{j,1,i}=max(forceDataNew{j,1,i})

    end
end
cellLeon=forceDataNew{:,3,1};
X=cell2mat(cellLeon(:));


%%
X=cell2mat(forceDataNew{:,3,1})
Y=cell2mat(forceDataNew{:,1,1})
% figure;
% plot(x,y,'k')
% % hold on;
% plot(forceDataNew{:,3,2},forceDataNew{:,1,2},'b')
% plot(forceDataNew{:,3,3},forceDataNew{:,1,3},'r')
% plot(forceDataNew{:,3,4},forceDataNew{:,1,4},'g')
% plot(forceDataNew{:,3,5},forceDataNew{:,1,5},'c')
% plot(forceDataNew{:,3,6},forceDataNew{:,1,6},'m')