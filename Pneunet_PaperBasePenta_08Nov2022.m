...
% m=0; n_count=0;
% %If all x coordinates if all the Vertices of a Face are in x=0 plane, set C
% %to 0
% for i=1:1:size(FO,1)
%     m=0; n_count=0;
%     for j=1:1:size(FO,2)
...
%         end
%          if m == size(FO,2)
%            CO(i)=-1; %Spine face to ID -1 face  
%          end
%          if n_count == size(FO,2)
%            CO(i)=0; %setting fixed face to ID 0 face  
%          end 
%     end
% end
...
%% SLL
Fsll_top=FO(ismember(CO,2),:);
% Fsll_top=Fsll_top-min(Fsll_top);
% indSLL=VO(:,2) == 0;
% Vsll=VO(indSLL,:);
Csll=(1:1:size(Fsll_top,1))';

layerThicknessSpat=SLL_thickness.*ones(size(VO,1),1);
pointSpacing=1;
...
% Csll2=Csll+1;
% Csll=[Csll; Csll2];
Csll =repmat(Csll,numSteps,1);

[Esll,VSLL,Fp1,Fp2]=patchThick(Fsll_top,VO,dirSet,layerThicknessSpat,numSteps); %Get mesh
...
[FSLL, CFSLL] = element2patch(Esll,Csll,'penta6');


%Get boundary faces (two sets due to pentahedra)
indb = tesBoundary(FSLL); %Cell containing boundary face indices
FbSLL = {FSLL{1}(indb{1},:),FSLL{2}(indb{2},:)}; %Cell containing boundary faces
CbSLL = {CFSLL{1}(indb{1},:),CFSLL{2}(indb{2},:)};
Eb1=patchBoundaryLabelEdges(FbSLL{1},VSLL,CbSLL{1}); %Boundary edges for visualization


%setting Face & Boundary Colours 
%Inputs
XYZdirection=[1;1;2;2;3;3];
XYZvalueSLL=[0 max(VSLL(:,1)); 0 max(VSLL(:,2)); 0 max(VSLL(:,3));];
...
for C_counter_SLL=1:1:size(XYZdirection,1)
%   xyzD= XYZdirection(C_counter_SLL)
%   xyzV= XYZvalueSLL(C_counter_SLL)
[CFSLL1] = XYZColourDesignate(FSLL1,VSLL,XYZdirection(C_counter_SLL),XYZvalueSLL(C_counter_SLL),0);

[CFSLL2] = XYZColourDesignate(FSLL2,VSLL,CFSLL2,XYZdirection(C_counter_SLL),XYZvalueSLL(C_counter_SLL),0);


[CbSLL1] = XYZColourDesignate(FbSLL1,VSLL,CbSLL1,XYZdirection(C_counter_SLL),XYZvalueSLL(C_counter_SLL),0);

[CbSLL2] = XYZColourDesignate(FbSLL2,VSLL,CbSLL2,XYZdirection(C_counter_SLL),XYZvalueSLL(C_counter_SLL),0);
...
gpatch(FbSLL,VSLL,CbSLL,'k',faceAlpha1); %Boundary faces
gedge(Eb1,VSLL,'k',4);
...
gpatch(FbSLL,VSLL,'gw','k',faceAlpha1); %Boundary faces
% patchNormPlot(F,V); %Visualise normal directions
axisGeom(gca,fontSize);
camlight headlight;
drawnow;

[FSLL,VSLL,CSLL]=joinElementSets(FSLL,VSLL);
cFigure; hold on; 
gpatch(FSLL,VSLL,CSLL);
patchNormPlot(FSLL,VSLL);
colormap spectral; icolorbar;
axisGeom; camlight headlight;
drawnow; 
...
[F,V,C]=joinElementSets({FO,FI,FSLL},{VO,VI,VSLL},{CO,CItemp,CSLLtemp});
