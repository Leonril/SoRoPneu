...
cFigure;
gpatch(Fi,V,'rw',1,0.5)
gpatch(Fo,V,'bw',1,0.5)
axisGeom


%% Using the surfaces for implicit geometry
contourLevel=0;% position in eq result where mesh should form
pointSpacingMean=mean(patchEdgeLengths(Fb,V));
voxelSize=pointSpacingMean;%/3;%approx size of voxels (mm)

imOrigin=[-1.9116   -1.9116   -1.9116]-voxelSize/2;% must be outside of the face

[L,G,bwLabels]=patch2Im(Fo,V,ones(size(Fo,1),1),voxelSize,imOrigin);
imSiz=size(L);
imOrigin=G.origin;
%[L,G,bwLabels]=patch2Im(Fi,V,ones(size(Fi,1),1),voxelSize,imOrigin,imSiz);

[J,I,K]=meshgrid(1:1:imSiz(2),1:1:imSiz(1),1:1:imSiz(3));
[X,Y,Z]=im2cart(I,J,K,voxelSize);

vizStruct.colormap=warmcold(250);
%vizStruct.clim=abs(max(M(:)))*[-1 1];
sv3(L,voxelSize,vizStruct); 
 %camlight headlight; 
hold on;
%plot3(X(:),Y(:),Z(:),'w.')

gpatch(Fo,V-imOrigin,'rw',1,0.5)
...
%% Outer surface modelling
% Plotting outer
Mo_grid=minDist(V_grid,V(indOuter,:));
Mo_grid(L==1)=-Mo_grid(L==1);

%M_grid=sin(nRepeat*Z)-sin(2.8*R)+2.8*R-6;% Coordinate to attach value to true 

Mo=reshape(Mo_grid,imSiz); 



vizStruct.colormap=warmcold(250);
vizStruct.clim=abs(max(Mo(:)))*[-1 1];
sv3(Mo,voxelSize,vizStruct); 
camlight headlight; 
...
[Fo,Vo]=levelset2isosurface(Mo,controlPar_isosurface); %Get iso-surface

Fo_sorted=sort(Fo,2);
logicInvalid=any(diff(Fo_sorted,1,2)==0,2);%remove collapsed tris
Fo=Fo(~logicInvalid,:);
[Fo,Vo]=patchCleanUnused(Fo,Vo);%coll tri nodes
Vo=Vo(:,[2 1 3]);
Vo=Vo+imOrigin;
[Fo,Vo]=triSurfRemoveThreeConnect(Fo,Vo);%remove tri connected
[Fo,Vo]=patchCleanUnused(Fo,Vo);

Vo=Vo-imOrigin; 
Vo=Vo(:,[2 1 3]);

%gpatch(Fo,Vo,'w','k',1,0.5);

%% Inner surface modelling

[L,G,bwLabels]=patch2Im(Fi,V,ones(size(Fi,1),1),voxelSize,imOrigin);
imSiz=size(L);
imOrigin=G.origin;
%[L,G,bwLabels]=patch2Im(Fi,V,ones(size(Fi,1),1),voxelSize,imOrigin,imSiz);
...
gpatch(Fi,V-imOrigin,'rw',1,0.5)

V_grid=[X(:) Y(:) Z(:)];


% Plotting outer
Mi_grid=minDist(V_grid,V(indInner,:));
Mi_grid(L==1)=-Mi_grid(L==1);

%M_grid=sin(nRepeat*Z)-sin(2.8*R)+2.8*R-6;% Coordinate to attach value to true 

Mi=reshape(Mi_grid,imSiz); 



vizStruct.colormap=warmcold(250);
vizStruct.clim=abs(max(Mi(:)))*[-1 1];
sv3(Mi,voxelSize,vizStruct); 
camlight headlight; 
...
controlPar_isosurface.voxelSize=voxelSize;
controlPar_isosurface.contourLevel=contourLevel;
[Fi,Vi]=levelset2isosurface(Mi,controlPar_isosurface); %Get iso-surface
...
%% Plotting to ensure no overlap
Ci=ones(size(Fi,1),1);
Co=ones(size(Fo,1),1)*2;

cFigure;
gpatch(Fi,Vi,Ci,'k',0.25);
hold on;
gpatch(Fo,Vo,Co,'k',0.25);
axisGeom; icolorbar;
...
[F,V,C]=joinElementSets({Fo,Fi},{Vo,Vi},{Co,Ci});

%Finding interior points
[V_regions]=getInnerPoint({Fo,Fi},{Vo,Vi}); 
[V_holes]=getInnerPoint(Fi,Vi); 

%Volume parameters
regionTetVolumes=tetVolMeanEst(Fo,Vo);
...
F_pressure=Fb(Cb==1,:);
bcSupportList=find(V(:,1)<(1.1*(min(V(:,1)))));

cFigure;
gpatch(Fb,V,'bw','k',0.2);
hold on;
hl(1)=gpatch(F_pressure,V,'r','k',1);
hl(2)=plotV(V(bcSupportList,:),'k.','MarkerSize',15);
legend(hl,{'Pressure surface','BC full support'});%'SLLsurface'
...
%%
cFigure;
gpatch(F_inner_original,V_original,'bw','k',0.5)
hold on;
gpatch(Fi,Vi+imOrigin,'rw','k',1)
axisGeom;
...
febioAnalysis.maxLogCheckTime=60; %Max log file checking time - EDITED 
