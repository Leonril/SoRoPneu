...
zposSLL=0; %z postion of the centre of the SLL, from the midpoint of the base thickness
SLL_thickness=0.1; %thickness of SLL
SLL_xpos=0.5; %where the SLL starts and ends, relative to full lenght
SLL_ypos=1; %SLL distance form wall width
...
% [FSLL,VSLL,boundaryIndSLL]=regionTriMesh2D(regionCell,pointSpacingSLL,resampleCurveOpt,0);
% 


inputStructure.regionCell={Vsll};
inputStructure.pointSpacing=pointSpacingSLL;
inputStructure.resampleCurveOpt=1; %Turn on/off curve resampling
inputStructure.plotOn=0;
inputStructure.mustPointsBoundary=Vsll; %Hold on to all boundary points
inputStructure.smoothIterations=50;

[FSLL,VSLL,boundaryIndSLL,indMustPointsBoundary]=regionTriMesh2D(inputStructure);
VSLL(:,3)=SLL_ypos;%adding Y value to V
VSLLb=VSLL(boundaryIndSLL,:);
...
dSLL=t-(2*SLL_ypos);%width
nsSLL=ceil(dSLL./pointSpacingSLL);%number of partitions in the sub
nsSLL=nsSLL+iseven(nsSLL);
...
cPar.dir=1;
cPar.closeLoopOpt=1; 
[FSLLt,VSLLt]=polyExtrude(VSLLb,cPar);

VSLL2=VSLL;
VSLL2(:,3)=t-SLL_ypos;%adding the Z value to V2

[FSLL,VSLL,CSLL]=joinElementSets({fliplr(FSLL),FSLL,fliplr(FSLLt)},{VSLL,VSLL2,VSLLt});
[FSLL,VSLL]=mergeVertices(FSLL,VSLL);
...
%% Colour IDs
CO=CO+1;%shifting all colour IDs up one
CItemp=CI+max(CO);
CSLLtemp=CSLL+max(CItemp);
