...
% Height Parameters
h_c=15; %height of the pneunet chambers (mm) from the base
rt=2;%roof thickness
ct=0.5;
...
cFigure; hold on;
plotV(VO,'b.-','MarkerSize',markerSize*2,'LineWidth',lineWidth/2); 
plotV(VI,'r.-','MarkerSize',markerSize*2,'LineWidth',lineWidth/2); 
axisGeom; view(2);
drawnow; 
...
cFigure;
gpatch(FI,VI,CItemp); hold on;
gpatch(FO,VO,CO,'k',0.5);
axisGeom;icolorbar;
...
[F,V,C]=joinElementSets({FO,FI},{VO,VI},{CO,CItemp});
...
%Get spine faces
FSLL=fliplr(Fb(Cb==2,:));%gets spine face

%Get pressure faces
F_pressure=Fb(ismember(Cb,[6 7 8]),:);  

%%
dirSet=1;
layerThickness=1;
numSteps=ceil(layerThickness./pointSpacing);
[Ep,Vp]=patchThick(FSLL,V,dirSet,layerThickness,numSteps);
Fp=element2patch(Ep,[],'penta6');

%% Join node sets

Ep=Ep+size(V,1); %Offset indices for penta6 
V=[V;Vp];
...
title('Tri - shell','FontSize',fontSize);
axisGeom(gca,fontSize);
camlight headlight; %icolorbar;

subplot(1,2,2);

title('Penta - thick','FontSize',fontSize);
gpatch(Fb,V,'r','k',0.1)
hold on;
hll=gpatch(Fp,V,'c')
...
febio_spec.Mesh.Elements{2}.elem.ATTR.id=size(E,1)+(1:1:size(Ep,1))'; %Element id's
febio_spec.Mesh.Elements{2}.elem.VAL=Ep; %The element matrix

partName3='Part3';
febio_spec.Mesh.Elements{3}.ATTR.name=partName3; %Name of this part
febio_spec.Mesh.Elements{3}.ATTR.type='tri3'; %Element type 
febio_spec.Mesh.Elements{3}.elem.ATTR.id=size(E,1)+size(Ep,1)+(1:1:size(FSLL,1))'; %Element id's
febio_spec.Mesh.Elements{3}.elem.VAL=FSLL; %The element matrix
...
febio_spec.MeshData.ElementData{1}.elem.VAL=SLLThickness*ones(size(FSLL,1),size(FSLL,2));
