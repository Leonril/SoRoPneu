...
pointSpacingList=[3 2 1.5 1 0.85 0.7];
%pointSpacingList=[1.2 1 0.8 0.7 0.6];

MaxBendAngNodes=zeros(numel(pointSpacingList),1);
MaxBendAngNormals=zeros(numel(pointSpacingList),1);
MaxTipForceMean=zeros(numel(pointSpacingList),1);
MaxTipForceTotal=zeros(numel(pointSpacingList),1);

for ConvergenceCounter=1:1:numel(pointSpacingList)
    close all; %closes any plots still open
...
n=4; %no. of chambers 
pointSpacing=pointSpacingList(ConvergenceCounter);%Only active if MeshCreationTool == 2, designated aproximate size of desired cube element length and width if possible (mm) 
...
febio_spec.Step.step{2}.ATTR.id=2; %fix tip and inflate further
...
%STEP 2 - Fix and increase pressure
% febio_spec.Step.step{2}.Boundary.bc{1}.ATTR.type='fix';
% febio_spec.Step.step{2}.Boundary.bc{1}.ATTR.node_set=nodeSetName2;
% febio_spec.Step.step{2}.Boundary.bc{1}.dofs='z';
...
% febio_spec.Loads.surface_load{2}.ATTR.surface=surfaceName1;
% febio_spec.Loads.surface_load{2}.pressure.ATTR.lc=2;
% febio_spec.Loads.surface_load{2}.pressure.VAL=2*appliedPressure;
...
febioAnalysis.maxLogCheckTime=100000; %Max log file checking time - EDITED 
...
YZnormVec=[1 0 0];
indT1=find(timeVec==1); %index at which step 2 starts
...
%% Collecting data for Mesh Convergence Output
%Filling matrixes
% MaxBendAngNodes(ConvergenceCounter)=max(BendingAngle(:,1));
% MaxBendAngNormals(ConvergenceCounter)=max(BendingAngle(:,1));
MaxBendAngNodes(ConvergenceCounter)=BendingAngle(indT1,1);
MaxBendAngNormals(ConvergenceCounter)=BendingAngle(indT1,2);
MaxTipForceMean(ConvergenceCounter)=max(Force_mean);
MaxTipForceTotal(ConvergenceCounter)=max(Force_total);


end % successful run check

end % End of PointSpacing loop

%% Plotting Mesh Convergence Results
%Turning to % to scale different parameters to one plot
MaxBendAngNodesSc=100*(MaxBendAngNodes/(MaxBendAngNodes(1)));
MaxBendAngNormalsSc=100*(MaxBendAngNormals/(MaxBendAngNormals(1)));
MaxTipForceMeanSc=100*(MaxTipForceMean/(MaxTipForceMean(1)));
MaxTipForceTotalSc=100*(MaxTipForceTotal/(MaxTipForceTotal(1)));

cFigure;
mc(1)=plot(pointSpacingList,MaxBendAngNodesSc,'r-');
hold on;
mc(2)=plot(pointSpacingList,MaxBendAngNormalsSc,'g-');
mc(3)=plot(pointSpacingList,MaxTipForceMeanSc,'y-');
mc(4)=plot(pointSpacingList,MaxTipForceTotalSc,'k');
legend(mc,{'Angle (node)','Angle (normal)','Force (mean)', 'Force (total)'});
title('Mesh Covergence (% of result of trial 1)')
